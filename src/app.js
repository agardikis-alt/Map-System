...
export class BoothMapSystem {
    ...
    _startTemplateAlignMode(group) {
        ...
        this._tplMoved = false; // tracks if mouse moved enough to count as drag
        ...
        // Drag threshold: must move >5px to count as drag (prevents accidental drags on click)
        const DRAG_THRESHOLD = 5;

        this._tplOnMd = (e) => {
            if (e.button !== 0) return;
            this._tplDragging = true;
            this._tplMoved = false;
        ...
        this._tplOnMm = (e) => {
            ...
            if (!this._tplMoved) {
                const dx = e.clientX - this._tplDownPos.x, dy = e.clientY - this._tplDownPos.y;
                if (Math.hypot(dx, dy) < DRAG_THRESHOLD) return; // haven't moved enough yet
                this._tplMoved = true;
            }
            e.preventDefault();
            const p = toSvg(e.clientX, e.clientY);
        ...
        // Click handler: if we didn't drag, place the template
        this._tplOnClick = (e) => {
            if (this._tplMoved) return; // was a drag, not a click
            this._finishTemplateAlign();
        };
        // Escape key to cancel
        this._tplOnKey = (e) => {
            if (e.key === 'Escape') this._cancelTemplateAlign();
        };
        ...
        // Use capture phase for click so it fires before booth clicks
        container.addEventListener('click', this._tplOnClick, true);
    ...
    _cleanupTemplateAlign() {
        this._templateAligning = false; this._tplDragging = false; this._tplMoved = false; this._templateGroup = null;
        const container = document.querySelector('.map-container');
        ...
        if (this._tplOnClick) container.removeEventListener('click', this._tplOnClick, true);
    ...
    updateStats(){const d=this.eventsData[this.currentEvent];if(!d?.booths)return;const b=Object.values(d.booths);document.getElementById('stat-total').textContent=b.length;document.getElementById('stat-assigned').textContent=b.filter(x=>x.boothStatus==='assigned').length;document.getElementById('stat-open').textContent=b.filter(x=>x.boothStatus==='open').length;document.getElementById('stat-unavailable').textContent=b.filter(x=>x.boothStatus==='unavailable').length;}
